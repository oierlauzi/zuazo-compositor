																			...
																			FillSide side ) const noexcept 
{
	...
	case CurveType::QUADRATIC: {
		result.klmCoords = {
			Zuazo::Math::Vec3<value_type>(0.0f, 		0.0f, 		0.0f		),
			Zuazo::Math::Vec3<value_type>(1.0f/3.0f, 	0.0f, 		1.0f/3.0f	),
			Zuazo::Math::Vec3<value_type>(2.0f/3.0f, 	1.0f/3.0f, 	2.0f/3.0f	),
			Zuazo::Math::Vec3<value_type>(1.0f, 		1.0f, 		1.0f		)
		};
		reverse = c.d3 < 0;
		break;
	}
	...
	case CurveType::SERPENTINE: {
		...
		const auto sqrt3DiscTerm1 = Math::sqrt(3*c.discriminantTerm1);
	...
	case CurveType::LOOP: {
		assert(c.discriminantTerm1 < 0); //To avoid NaNs
		const auto sqrtDisc = Math::sqrt(-c.discriminantTerm1);
		...
		if(Math::isInRange(ql, value_type(0), value_type(1))) {
			result.subdivisionParameter = ql;
		} else if(Math::isInRange(qm, value_type(0), value_type(1))) {
			...
			reverse = Math::sign(result.klmCoords.front().x) != Math::sign(c.d1);
		}
	...
	if(reverse) {
		for(size_t i = 0; i < result.klmCoords.size(); ++i) {
			result.klmCoords[i] *= Math::Vec3<value_type>(-1, -1, 1);
		}
	}
